...
function readForm(
  ...
  const raw = projectData?.despesasEventoFinal
  ...
  const despesas = Array.isArray(data.despesas)
    ? data.despesas
        ...
        .map((row, index) => ({
          id: String(row.id ?? `evento-final-${index}`),
...
export function useDespesasEventoFinal(_options: Options = {}) {
